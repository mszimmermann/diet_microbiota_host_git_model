function [changingMets_merged,...
          ...
          changingMets_merged_mode] = merge_changing_metabolites(changingMets,...
                                                                   ...
                                                                   varargin)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% if charge is provided, include it in spectrum comparison
chargeMass = 1.007825;
if nargin>2
    chargeVector = varargin{1};
else
    chargeVector=zeros(size(changingMets));
end
...
for i=1:length(changingMets_merged)
    if changingMets_merged(i)==0
        ...
        if length(sameidx)>1
            ...
            % get charges to compare composite spectra
            curCharges = chargeMass*chargeVector(sameidx); % dont correct for charge 
            ...
            baseSpectrum = baseSpectrum-curCharges(sameidx==i);
            ...
            for j=1:length(curSpectra)
                ...
                curSpectraMZ = curSpectraMZ-curCharges(j);
            ...
            % only merge the same charge
            sameidx = sameidx((spectrumOverlap>1) & (curCharges==curCharges(sameidx==i)));
        end
...
changingMets_merged_mode = zeros(size(changingMets_merged_idx_unique));
...
for i=1:length(changingMets_merged_idx_unique)
    ...
    changingMets_merged_mode(i) = chargeVector(merged_idx(med_idx));
