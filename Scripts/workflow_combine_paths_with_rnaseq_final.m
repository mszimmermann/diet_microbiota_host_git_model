...
print(gcf, '-vector', '-dpdf', '-r600', '-bestfit', ...
   [figureFolder ...
   'fig5c_sankey_productcorr_withtext_strictclass'...
    num2str(strict_class), '.pdf'])
...
writetable(curenzymes_locustags_combined, ...
    '.\ProcessedData\shortBRED\locustags_for_plotting_products_040126_noneg.txt');
...
% calculate adjusted correlation p-values
met_enz_PCCPFDRall = my_bhfdr(met_enz_PCCPall);
met_enz_SCCPFDRall = my_bhfdr(met_enz_SCCPall);
met_enz_PCCPFDRdc = my_bhfdr(met_enz_PCCPdc);
met_enz_SCCPFDRdc = my_bhfdr(met_enz_SCCPdc);
met_enz_PCCPFDRcvr = my_bhfdr(met_enz_PCCPcvr);
met_enz_SCCPFDRcvr = my_bhfdr(met_enz_SCCPcvr);
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% make a table of shortbred correlations as output
met_enz_corr_table = table(met_enz_metabolite, met_enz_enzyme,...
    met_enz_PCCall, met_enz_PCCPall, met_enz_PCCPFDRall,...
    met_enz_PCCdc, met_enz_PCCPdc, met_enz_PCCPFDRdc,...
    met_enz_PCCcvr, met_enz_PCCPcvr, met_enz_PCCPFDRcvr,...
    met_enz_NMETall, met_enz_NENZPall,...
    met_enz_NMETdc, met_enz_NENZdc,...
    met_enz_NMETcvr, met_enz_NENZcvr);
% add data to the columns
for i=1:size(met_enz_metabolite_data,2)
    met_enz_corr_table = addvars(met_enz_corr_table, met_enz_metabolite_data(:,i),...
        'NewVariableNames', [shortbred_columns{i}, '_met_data']);
end
for i=1:size(met_enz_enzyme_data,2)
    met_enz_corr_table = addvars(met_enz_corr_table, met_enz_enzyme_data(:,i),...
        'NewVariableNames', [shortbred_columns{i}, '_enzyme_data']);
end

writetable(met_enz_corr_table,...
    [outputFolder,...
    'tableS15_met_enz_correlations_shortbred_data.csv']);
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
