...
% %For DC clustergram
% save([outputFolder clustergramFile],...
% 'cgo', 'cgoHighHostColon', 'cgoHColhfd', 'cgoHSI_only', 'cgoHSI_and_mix',...
% 'cgoBLIneg', 'cgoHLIhfd', 'cgoHLIctr', 'cgoBLIctr', 'cgoBLIhfd',...
% 'cgoHcolhigh_and_mixed', 'cgoHLIctr_neg', 'cgoBLIctr_neg',...
% 'cgoHLIhfd_neg');
% % for CVR clustergram
% save([outputFolder clustergramFile],...
% 'cgo', 'cgoHSIonly', 'cgoHSImixed_all',...
% 'cgoHColhigh', 'cgoHLIhfd_neg', 'cgoHLIctr_neg',...
% 'cgoBLIctr', 'cgoBLIhfd', 'cgoHLIctr_mixed', ...
% 'cgoHLIhfd_mixed', 'cgoBLIhfd_neg');
...
if isequal(analyzeGroup, 'DC')
    ...
    cluster_order = [{cellfun(@(x) (str2double(x)), cgoHColhfd.ColumnLabels)},...
            ...
            {setdiff(cellfun(@(x) (str2double(x)), cgoHSI_and_mix.ColumnLabels),...
                     cellfun(@(x) (str2double(x)), cgoHSI_only.ColumnLabels))},...
            {cellfun(@(x) (str2double(x)),cgoBLIneg.ColumnLabels)},...
            ...
            {cellfun(@(x) (str2double(x)),cgoHcolhigh_and_mixed.ColumnLabels)},...
    ...
    extra_subcluster1 = cellfun(@(x) (str2double(x)), cgo.ColumnLabels(idx(1:5)));
    extra_subcluster2 = cellfun(@(x) (str2double(x)), cgo.ColumnLabels(idx(6:124)));
    extra_subcluster3 = cellfun(@(x) (str2double(x)), cgo.ColumnLabels(idx(125:end)));
    cluster_order = [cluster_order(1:5), {extra_subcluster1}, ...
                     cluster_order(6:9), {extra_subcluster2}, ...
                     cluster_order(10:end), {extra_subcluster3}];
    
    cluster_sizes = cellfun(@(x) length(x), cluster_order);
    ...
    cluster_order = [{cellfun(@(x) (str2double(x)), cgoHSIonly.ColumnLabels)},...
            ...
            {cellfun(@(x) (str2double(x)),cgoBLIctr.ColumnLabels)},...
            {cellfun(@(x) (str2double(x)),cgoBLIhfd.ColumnLabels)},...
            {cellfun(@(x) (str2double(x)),cgoHLIctr_mixed.ColumnLabels)},...
            {cellfun(@(x) (str2double(x)),cgoHLIhfd_mixed.ColumnLabels)},...
            {cellfun(@(x) (str2double(x)),cgoBLIhfd_neg.ColumnLabels)}];
    ...
    % find missing ones and add them manually
    [~, idx] = setdiff(cellfun(@(x) str2double(x), cgo.ColumnLabels), cluster_subcluster_idx, 'stable');
    extra_subcluster1 = cellfun(@(x) (str2double(x)), cgo.ColumnLabels(idx(1:124)));
    extra_subcluster2 = cellfun(@(x) (str2double(x)), cgo.ColumnLabels(idx(125:138)));
    extra_subcluster3 = cellfun(@(x) (str2double(x)), cgo.ColumnLabels(idx(139:end)));
    
    cluster_order = [cluster_order(1:3), {extra_subcluster1}, ...
                     cluster_order(4:5), {extra_subcluster2},...
                     cluster_order(6:end), {extra_subcluster3}];
