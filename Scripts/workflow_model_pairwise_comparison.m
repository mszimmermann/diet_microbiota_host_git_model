...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% make a table with <eier, DC and CVR coeffifients for intersecting
% metabolites alltogether
model_comparison_table = table(met_info_intersect_meier.MZ, 'VariableNames', {'MZ'});
model_comparison_table = addvars(model_comparison_table, ...
    met_info_intersect_meier.RT, 'NewVariableNames', {'RT'});
model_comparison_table = addvars(model_comparison_table, ...
    met_info_intersect_meier.CompoundName, 'NewVariableNames', {'CompoundName'});
% add meier model
model_comparison_table = addvars(model_comparison_table, ...
    met_bestsol_meier.selection_criteria, 'NewVariableNames', {'SolutionType_Meier'});
model_comparison_table = addvars(model_comparison_table, ...
    met_bestsol_meier.x_sel_CorrRev, 'NewVariableNames', {'ReciprocalCorr_Meier'});
for i=1:size(met_bestsol_meier.x,2)
    model_comparison_table = addvars(model_comparison_table, ...
        met_bestsol_meier.x(:,i), 'NewVariableNames', ...
        strcat(met_bestsol_meier.coefvalues{i}, '_Meier'));
end
for i=2:size(met_bestsol_meier.x,2)
    model_comparison_table = addvars(model_comparison_table, ...
        met_bestsol_meier.x(:,i)./max(abs(met_bestsol_meier.x(:,2:end)),[],2), 'NewVariableNames', ...
        strcat(met_bestsol_meier.coefvalues{i}, '_Meier_norm_by_max_abs'));
end
% add DC model
model_comparison_table = addvars(model_comparison_table, ...
    met_bestsol_combined_DC_filtered.selection_criteria, 'NewVariableNames', {'SolutionType_DC'});
model_comparison_table = addvars(model_comparison_table, ...
    met_bestsol_combined_DC_filtered.x_sel_CorrRev, 'NewVariableNames', {'ReciprocalCorr_DC'});
for i=1:size(met_bestsol_combined_DC_filtered.x,2)
    model_comparison_table = addvars(model_comparison_table, ...
        met_bestsol_combined_DC_filtered.x(:,i), 'NewVariableNames', ...
        strcat(met_bestsol_combined_DC_filtered.coefvalues{i}, '_DC'));
end
for i=2:size(met_bestsol_combined_DC_filtered.x,2)
    model_comparison_table = addvars(model_comparison_table, ...
        met_bestsol_combined_DC_filtered.x(:,i)./max(abs(met_bestsol_combined_DC_filtered.x(:,2:end)),[],2), 'NewVariableNames', ...
        strcat(met_bestsol_combined_DC_filtered.coefvalues{i}, '_DC_norm_by_max_abs'));
end
% add CVR model
model_comparison_table = addvars(model_comparison_table, ...
    met_bestsol_combined_CVR_filtered.selection_criteria, 'NewVariableNames', {'SolutionType_CVR'});
model_comparison_table = addvars(model_comparison_table, ...
    met_bestsol_combined_CVR_filtered.x_sel_CorrRev, 'NewVariableNames', {'ReciprocalCorr_CVR'});
for i=1:size(met_bestsol_combined_CVR_filtered.x,2)
    model_comparison_table = addvars(model_comparison_table, ...
        met_bestsol_combined_CVR_filtered.x(:,i), 'NewVariableNames', ...
        strcat(met_bestsol_combined_CVR_filtered.coefvalues{i}, '_CVR'));
end
for i=2:size(met_bestsol_combined_CVR_filtered.x,2)
    model_comparison_table = addvars(model_comparison_table, ...
        met_bestsol_combined_CVR_filtered.x(:,i)./max(abs(met_bestsol_combined_CVR_filtered.x(:,2:end)),[],2), 'NewVariableNames', ...
        strcat(met_bestsol_combined_CVR_filtered.coefvalues{i}, '_CVR_norm_by_max_abs'));
end
% print table to file
writetable(model_comparison_table,...
    [outputFolder,...
    'tableS13_model_coef_comparison_Meier_DC_CVR.csv']);
